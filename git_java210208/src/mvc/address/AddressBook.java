...
public class AddressBook extends JFrame {
	Logger logger = (Logger)LogManager.getLogger(AddressBook.class);
    ...
    public static void main(String args[]) {
    	System.setProperty(XmlConfigurationFactory.CONFIGURATION_FILE_PROPERTY,"C:\\git_java80\\git_java210208\\git_java210208\\log4j.xml");
        abook = new AddressBook(); //1.얘가 제일 먼저 시작 ->54번줄
        abook.setVisible(true);//3. 화면 보임
    }

    // 생성자는 컴포넌트들을 초기화합니다.
    public AddressBook() {
        initComponents(); //2.UI교체 가능(web은 -> html/ 안드로이드는 -> xml) =>49번줄
    }
    ...
    private void initComponents() {
		...
		btnDelete.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
            	//evt JVM이 재사용 파라리터 - 공통코드(ex)deleteActionPerformed()이런 것들)를 사용하므로써 재사용이 가능해진다/
            	logger.info("삭제 아이콘");
            	deleteActionPerformed();
            }
        });
...
3. intComponent()
   아직 setvisible(ture)
4.refreshData() --> AddressCrtl -> send, sendAll
*/