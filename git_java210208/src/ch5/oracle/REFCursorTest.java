 ...
 * IS
   ...
   JDBC연동 기술은 공통된 관심사이다.
    매번 동시 접속자 수가 많을 수 있고 다중 처리를 해야 하므로 DB커넥션 풀링을 고려해야 한다. - tomcat에서 제공되는 풀링이다. HikariCP도 나왔는데 이것도 풀링이 빠르다.
 */
public class REFCursorTest {
	...
	public static void main(String[] args) { //main함수 자체가 entry point -default thread  - 순서도 절차 지향
		//synchronized가 너무 느려서 --> 객체 풀링 등장(버려지는 객체를  버리지말고 다시 꺼내 쓰는 것 - 다시 꺼내써서 더럽겠지. 그래서 clean후에 재사용)
	...
	public void init() {
		dbMgr = DBConnectionMgr.getInstance();//3번 싱글톤
