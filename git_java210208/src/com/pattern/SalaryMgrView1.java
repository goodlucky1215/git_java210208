 ...
 * 생성자의 파라미터 자리에는 무엇을 써야 할까요? - 생성자 오버로딩에 대해서는 알고 있는 거니?
 * SalaryMgrView - this   (initDisplay(), main())
 * SalaryMgrHandler		  (actionPerformed)
 * SalaryMgrLogic		  (getEmpDetail)
 * 한개만, 두개가, 세개 다 원하는지
 * 세 개의 클래스를 그림으로 표시한 뒤 화살표를 통해서 객체 주입관계를 완성하시오.
 * Q1.이벤트 소스는 어느 클래스에서 가져와야 할까요?	          SalaryMgrView
 * Q2.addActionListener메소드는 어디에서 호출되야 하나요?	  SalaryMgrView
 * Q3.addActionListener(this)메소드는 어디에서 호출되야 하나요?	  SalaryMgrHandler
 * Q4.getEmpDetail메소드는 어느 클래스에서 어떤 메소드에서 호출하나요?  SalaryMgrHandler		actionPerformed에서 호출 되어야 함.
 */
public class SalaryMgrView1 {
		//선언부
		SalaryMgrHandler1  smEvent   = null;
		JFrame			  jf_sal 	= null;
	   ...
	   //생성자
	   public SalaryMgrView1() {
		  smEvent   = new SalaryMgrHandler1(this);
