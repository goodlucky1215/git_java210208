...
public class TomatoClientVer2 extends JFrame implements ActionListener{
	...
	JButton jbtn_fontSize	  = new JButton("글씨크기");
	...
	StyledDocument sd_display = new DefaultStyledDocument(
											new StyleContext());
	...
	public static void main(String args[]) {
		...
		//생성자 호출(this) - 듣기 가능해짐. - 전제 조건, oos, ois, 소켓 객체 있어야 가능함.
		tc.init();
	}
	//소켓 관련 초기화
	public void init() {
		try {
			...
			TomatoClientThreadVer2 tct = new TomatoClientThreadVer2(this);
			tct.start();
		} catch (Exception e) {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(jbtn_one == obj) {
			...
			//이렇게 하면 null을 맞을 수도 있다. 그래서 위에 처럼 null인지를 먼저 확인해야한다.
			//if(afterName.length()<3 || afterName==null) {}
			try {
				oos.writeObject(Protocol.CHANGE
						   +Protocol.seperator+nickName
						   +Protocol.seperator+afterName
						   +Protocol.seperator+"님의 대화명이 "+afterName+" 으로 변경");
				jtf_msg.setText("");
			} catch (Exception e) {
