...
/*
 * 클라이턴트 측에서 접속 시도가 있을 때 서버 측에서 클라이언트의 로그 정보를 보고 싶다.
 * 따라서 화면에 전광판을 하나 추가하여 텍스트로 로그 정보를 확인할 수 있도록 하기 위해서
 * JFrame을 상속 받았다.
 * mian메소드가 있는 클래스는 디폴트 스레드를 가진다.
 * 이 스레드에서 소켓 서버 정보를 관리하는것이 경합이 일어날 수 있고 그에 따라 충돌이나 이상이 발생
...
public class TalkServer extends JFrame  implements Runnable{
	...
	public void run() {
		...
		try {
			...
			//예외 발생에 대한 메시지들을 stack영역에서 간리하는데 이것들을 출력해주는 메소드입니다.
			//라인번호와 호출 순서에 대한 내용까지도 포함하니깐 더 많은 힌트를 얻을 수가 있습니다.
		}
	}
