...
public class AddressDialog2 extends JDialog implements ActionListener {
   ...
   StringBuilder sql_ins = null; //싱글스레드에 안전. 속도 빠름빠름
   /*
    *  UPDATE dept
          SET loc = '인천'
         WHERE deptno = 51
   ...
   //화면을 처리할때 해당 옵션을 두 가지로 설정가능함. - setter/getter형식으로 코딩해보기
    ...
    * 입력일때는 새창을 열어서 입력 받아야 함.
    * 수정일때는 오라클을 경유한 후 그 결과를 화면에 출력해 놓고 수정할 정보만
    * 새로 입력을 받는다.
    * 상세조회일때도 오라클을 경유하여 한 건만 출력을 해야 함. -readonly\
    * 수정과 입력일때는 writeable
    * 수정과 상세조회를 하나로 묶고
    * 입력을 하나로 묶어서 처리
    * 오라클을 경유했을때 VO 유지해야 함.
   ...
   public void set(String title, DeptVO dVO, AddressBook2 aBook, boolean isFlag) {
     con = dbMgr.getConnection();
      this.aBook = aBook;//전변
      this.dVO = dVO;//전변
      this.setTitle(title);//굳이 전변이 아니어도 괜찮아
   ...
   private void setValue(DeptVO dVO) {
      //입력을 위한 다이얼로그창 설정 - 모든 값을 null로 셋팅한다.
   ...
   //set메소드를 통해서 넘어온 4번째 값에 따라서 화면을 처리하는 콤퍼넌트 클래스의 수정 모드에 대한 설정을 바꾸어 줍니ㅏㄷ.
   //만일 true이면 setEnabled(true)로 설정하고
   //만일 false가 넘어오면 setEnabled(flase)로 설정하여 수정할 수 없도록 할 계획인거죠
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if("처리".contentEquals(command)) {
         //입력 혹은 수정처리
         if(dVO==null) {//입력일때
            DeptVO pdVO = new DeptVO();
            pdVO.setDeptno(Integer.parseInt(getJtf_deptno()));//NumberFormatException 일어날 수 있음
            ...
            try {
               System.out.println("두번째1:됨");
               upstmt = con.prepareStatement(sql_upd.toString());
               int i = 0; //물음표 갯수만큼 필요해요. 시작은 1부터입니다.
               System.out.println("두번째2:됨");
               upstmt.setString(++i, pdVO.getLoc());
               System.out.println("두번째3:됨");
               upstmt.setInt(++i, pdVO.getDeptno());
               System.out.println("두번째4:됨");
               int uresult = upstmt.executeUpdate();
               System.out.println("두번째5:됨");
               if(uresult==1) {
